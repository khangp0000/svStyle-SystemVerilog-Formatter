...
#define INIT_CONTAINER(container, value)         \
  {                                              \
 ...
 */
ASFormatter::~ASFormatter() { DELETE_CONTAINER(preBracketHeaderStack); }

/**
 * initialization of static data of ASFormatter.
 */
void ASFormatter::staticInit() {
  if (calledInitStatic) return;
...
string ASFormatter::nextLine() {
  ...
  if (!isFormattingEnabled()) return ASBeautifier::nextLine();
  ...
  while (!isLineReady) {
    ...
    {
      // make sure that a virgin '{' at the begining ofthe file will be treated
      // as a block...
      if (isInVirginLine && currentChar == '{') previousCommandChar = '{';
    ...
    if (isSequenceReached(AS_OPEN_LINE_COMMENT)) {
      ...
      if (shouldPadOperators) appendSpacePad();
      ...
      ////if (shouldPadOperators)  // BUGFIX: these two lines removed. seem to
      /// be unneeded, and interfere with L" /appendSpacePad();    // BUFFIX:
      /// TODO make sure the removal of these lines doesn't reopen old bugs...
      appendCurrentChar();
      continue;
    }
    ...
    if (currentChar == PREPROCESSOR_CHAR &&
        (findHeader(preprocessorHeaders) != NULL))
    ...
    if (isImmediatelyPostHeader) {
      ...
      if (shouldBreakOneLineStatements) {
        ...
        if (shouldBreakElseIfs)
          ...
          // add by renqh, insert 'begin' , changed '   if(1) a; ' to ' if(1)
          // begin a;
    ...
    if (currentChar == '{') {
      ...
      if (currentChar == 'e' ||
          currentChar == 'j')  // join end endtable endspecify
      {
        ...
        if (vBlockEnd) {
          ...
          charNum = charNum + (vBlockEnd->length() - 1);
                  ...
                  currentChar == 's' ||
                  currentChar == 'b'))  // fork table specify begin
      {
        vBlockBegin = findHeader(verilogBlockBegin);
        if (vBlockBegin) {
          currentChar = '{';
    ...
    if (bracketFormatMode != NONE_MODE) {
      if (currentChar == '{') {
        if (bracketFormatMode == ATTACH_MODE &&
          ...
          appendSpacePad();
          if (!isCharImmediatelyPostComment  // do not attach '{' to lines that
                                             // end with /**/ comments.
          ...
          if (shouldBreakOneLineBlocks) breakLine();
          appendBlock();
          continue;
        }
        appendBlock(true);
        ...
        if (shouldBreakOneLineBlocks) {
          ...
          if (!isCharImmediatelyPostComment) isInLineBreak = false;
          appendBlock();
          if (shouldBreakOneLineBlocks) shouldBreakLineAfterComments = true;
        }
    ...
    if (((previousCommandChar == '{') ||
         (previousCommandChar == '}' &&
          !isPreviousCharPostComment  // <-- Fixes wrongly appended newlines
                                      // after '}' immediately after comments...
                                      // 10/9/1999
          && peekNextChar() != ' ')) &&
    ...
    if ((newHeader = findHeader(headers)) != NULL) {
      ...
      if (shouldPadOperators && !isNonParenHeader) appendSpacePad();
      ...
      if (shouldBreakBlocks) {
        if (previousHeader == NULL && !isCharImmediatelyPostOpenBlock) {
          isPrependPostBlockEmptyLineRequested = true;
        }

        if (currentHeader == &AS_ELSE || isCharImmediatelyPostLineComment ||
    ...
    if (previousNonWSChar == '}' || currentChar == ';') {
      ...
      if (currentChar != ';') currentHeader = NULL;  // DEVEL: is this ok?
    ...
    if (currentChar == ':') {
      if (previousNonWSChar == '}' || previousNonWSChar == '{') {
        ...
        continue;
      } else if (shouldBreakOneLineStatements && previousNonWSChar != '}' &&
                 previousNonWSChar != '{' &&
                 !foundQuestionMark  // not in a ... ? ... : ... sequence
                 && previousCommandChar !=
                        ')'  // not immediately after closing paren of a method
                             // header, e.g. ASFormatter::ASFormatter(...) :
                             // ASBeautifier(...)
      ) {
        passedColon = true;
      }
    ...
    if (currentChar == '?') {
      if (!isInVerilogNum(currentLine, charNum)) foundQuestionMark = true;
    }

    if (shouldPadOperators) {
      if ((newHeader = findHeader(operators)) != NULL) {
        bool shouldPad =
            (newHeader != &AS_PAREN_PAREN && newHeader != &AS_BLPAREN_BLPAREN &&
             newHeader != &AS_NOT && newHeader != &AS_BIT_NOT &&
             newHeader != &AS_BITNOT_AND && newHeader != &AS_BITNOT_OR &&
             newHeader != &AS_NOT_XNOR && newHeader != &AS_BITNOT_XNOR &&
        ...
        // pad before operator
        if (shouldPad && !(newHeader == &AS_COLON && !foundQuestionMark) &&
            newHeader != &AS_SEMICOLON && newHeader != &AS_COMMA)
          appendSpacePad();
...
void ASFormatter::setBlockPaddingMode(bool state) { shouldPadBlocks = state; }
...
/**
 * set option to break closing header blocks of code (such as 'else', 'catch',
 * ...) with empty lines.
 *
 * @param state        true = convert, false = don't convert.
 */
...
/**
 * check if a specific sequence exists in the current placement of the current
 * line
 *
...
bool ASFormatter::isBeforeComment() const {
  ...
  for (peekNum = charNum + 1;
       peekNum < len && isWhiteSpace(currentLine[peekNum]); ++peekNum)
    ;
...
bool ASFormatter::getNextChar() {
  ...
  if (shouldPadOperators && !isInComment && !isInLineComment && !isInQuote &&
      !doesLineStartComment && !isInPreprocessor && !isBeforeComment()) {
  ...
  if (charNum + 1 < currentLineLength &&
    ...
    if (shouldConvertTabs && currentChar == '\t') currentChar = ' ';
    ...
    if (sourceIterator->hasMoreLines()) {
      ...
      if (isInLineComment) isImmediatelyPostLineComment = true;
      isInLineComment = false;

      trimNewLine();
      currentChar = currentLine[charNum];

      // check if is in preprocessor right after the line break and line
      // trimming
      if (previousNonWSChar != '\\') isInPreprocessor = false;
...
void ASFormatter::trimNewLine() {
  ...
  if (isInComment || isInPreprocessor) return;

  while (isWhiteSpace(currentLine[charNum]) && charNum + 1 < len) ++charNum;
 ...
 */
BracketType ASFormatter::getBracketType() const { return COMMAND_TYPE; }
...
bool ASFormatter::isUrinaryMinus() const {
  return ((!isalnum(previousCommandChar)) && previousCommandChar != '.' &&
          previousCommandChar != ')' && previousCommandChar != ']');
}
...
bool ASFormatter::isOneLineBlockReached() const {
  ...
  for (i = charNum + 1; i < currentLineLength; ++i) {
    ...
    if (isInQuote) {
      if (ch == quoteChar) isInQuote = false;
      continue;
    }
    ...
    if (currentLine.COMPARE(i, 2, "//") == 0) break;
 ...
 *
 * @return             a pointer to the found header. Or a NULL if no header has
 * been reached.
 * @param headers      a vector of headers
 * @param checkBoundry
 */
const string *ASFormatter::findHeader(const vector<const string *> &headers,
                                      bool checkBoundry) {
