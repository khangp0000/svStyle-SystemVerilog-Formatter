...
void ASBeautifier::initStatic() {
  ...
  // SystemVerilog
  verilogBlockBegin.push_back(&AS_RANDCASE);
  verilogBlockBegin.push_back(&AS_CLASS);
  verilogBlockBegin.push_back(&AS_CLOCKING);
  verilogBlockBegin.push_back(&AS_COVERGROUP);
  verilogBlockBegin.push_back(&AS_INTERFACE);
  verilogBlockBegin.push_back(&AS_PACKAGE);
  verilogBlockBegin.push_back(&AS_PROGRAM);
  verilogBlockBegin.push_back(&AS_PROPERTY);
  verilogBlockBegin.push_back(&AS_SEQUENCE);
  verilogBlockBegin.push_back(&AS_RANDSEQUENCE);
  ...
  // SystemVerilog
  verilogBlockEnd.push_back(&AS_JOIN_ANY);
  verilogBlockEnd.push_back(&AS_JOIN_ALL);
  verilogBlockEnd.push_back(&AS_ENDCLASS);
  verilogBlockEnd.push_back(&AS_ENDCLOCKING);
  verilogBlockEnd.push_back(&AS_ENDGROUP);
  verilogBlockEnd.push_back(&AS_ENDINTERFACE);
  verilogBlockEnd.push_back(&AS_ENDPACKAGE);
  verilogBlockEnd.push_back(&AS_ENDPROGRAM);
  verilogBlockEnd.push_back(&AS_ENDPROPERTY);
  verilogBlockEnd.push_back(&AS_ENDSEQUENCE);
...
void ASBeautifier::setTabIndentation(int length, bool forceTabs) {
  ...
  if (!isMinimalConditinalIndentSet) minConditionalIndent = indentLength * 2;
}
...
void ASBeautifier::setBlockIndent(bool state) {
  if (state)
    setBracketIndent(
        false);  // so that we don't have both bracket and block indent
  blockIndent = state;
}
...
string ASBeautifier::beautify(const string &originalLine) {
  ...
  if (!isInComment && (isPreprocesor || backslashEndsPrevLine)) {
    if (line[0] == PREPROCESSOR_CHAR) {
      ...
      //    of the #define. This clone is put into the activeBeautifierStack in
      //    order to be called for the actual indentation.
      // The original beautifier will have isInDefineDefinition = true,
      // isInDefine = false The cloned beautifier will have isInDefineDefinition
      // = true, isInDefine = true
      if (preprocessorIndent && preproc.COMPARE(0, 6, string("define")) == 0 &&
          line[line.length() - 1] == '\\') {
        if (!isInDefineDefinition) {
          ...
          // the is the cloned beautifier that is in charge of indenting the
          // #define.
          isInDefine = true;
        }
  ...
  for (i = 0; i < iLength; (i = i + 1, isFirstLoop = false)) {
    ...
    if (!(isInComment || isInLineComment) &&
        line.COMPARE(i, 2, AS_OPEN_LINE_COMMENT) == 0) {
      isInLineComment = true;
      outBuffer.append(1, '/');
      i++;
      continue;
    } else if (!(isInComment || isInLineComment) &&
               line.COMPARE(i, 2, AS_OPEN_COMMENT) == 0) {
      isInComment = true;
      outBuffer.append(1, '*');
      i++;
      continue;
    } else if ((isInComment || isInLineComment) &&
               line.COMPARE(i, 2, AS_CLOSE_COMMENT) == 0) {
      isInComment = false;
      outBuffer.append(1, '/');
      i++;
      continue;
    ...
    if (isInComment || isInLineComment) continue;
    ...
    if (vBlockEnd != NULL) {
      ...
      ch = '}';
      if (vBlockEnd == &AS_ENDCASE) isInCase--;
    } else if (vBlockBegin != NULL) {
      outBuffer.append(vBlockBegin->substr(1));
      i = i + vBlockBegin->length() - 1;
      ch = '{';
      if (vBlockBegin == &AS_CASE || vBlockBegin == &AS_CASEZ ||
          vBlockBegin == &AS_CASEX)
        isInCase++;
    }
      ...
      if (ch == '(' || ch == '[') {
        ...
        if (!inStatementIndentStackSizeStack->empty()) {
          ...
          if (!parenIndentStack->empty()) {
            ...
            if (i == 0 || isFirstLoop) spaceTabCount = poppedIndent;
          }
        }
      }
      continue;
        ...
        // take care of 'else'
          ...
          if (lastTempStack != NULL) {
            ...
            if (indexOfIf != -1) {
              ...
              if (!closingBracketReached) tabCount += restackSize;
            }
          }
        }
    ...
    if ((ch == ';' && parenDepth == 0) ||
      ...
      if (ch == '}') {
        ...
        ch = ' ';  // needed due to cases such as '}else{', so that headers
    ...
    if (!isInStatement && i == 0 && parenDepth == 0 && isInCase == 0) {
      int pos = 0;
      while (pos < iLength) {
        if (!isLegalNameChar(line[pos])) break;
        ++pos;
      }
      if (pos >= iLength) pos--;
      while (pos < iLength) {
        if (!isWhiteSpace(line[pos])) break;
        ++pos;
      }
      if (pos >= iLength) pos--;

      registerInStatementIndent(string(line.c_str() + pos), 0, spaceTabCount,
                                pos, false);
      isInStatement = true;
    }
  }
  ...
  if (!lineStartsInComment && !blockIndent && outBuffer.length() > 0 &&
    ...
    --tabCount;
           ...
           (vBlockEnd != NULL) && shouldIndentBrackettedLine)
    --tabCount;
  ...
  if (bracketIndent && outBuffer.length() > 0 && shouldIndentBrackettedLine)
    if (vBlockEnd != NULL || vBlockBegin != NULL) tabCount++;
...
string ASBeautifier::preLineWS(int spaceTabCount, int tabCount) {
  ...
  for (int i = 0; i < tabCount; i++) ws += indentString;

  while ((spaceTabCount--) > 0) ws += string(" ");
...
void ASBeautifier::registerInStatementIndent(const string &line, int i,
  ...
  // if indent is around the last char in the line, indent instead 2 spaces from
  // the previous indent
  ...
  if (updateParenStack) parenIndentStack->push_back(i + spaceTabCount);
...
/**
 * check if a specific character can be used in a legal variable/method/class
 * name
 *
...
bool ASBeautifier::isLegalNameChar(char ch) const {
  return (isalnum(ch)  //(ch>='a' && ch<='z') || (ch>='A' && ch<='Z') ||
                       //(ch>='0' && ch<='9') ||
          || ch == '.' || ch == '_' || (ch == '$') || (ch == '`'));
}
/**
...
/**
 * check if a specific line position contains a header, out of several possible
 * headers.
 *
 * @return    a pointer to the found header. if no header was found then return
 * NULL.
 */
const string *ASBeautifier::findHeader(
    const string &line, int i, const vector<const string *> &possibleHeaders,
    bool checkBoundry) {
  ...
  for (p = 0; p < maxHeaders; p++) {
    ...
    if (line.COMPARE(i, header->length(), *header) == 0) {
      ...
      if (!checkBoundry) {
        return header;
      } else if (prevCh != 0 && isLegalNameChar(startCh) &&
                 isLegalNameChar(prevCh)) {
        continue;
      } else if (headerEnd >= lineLength || !isLegalNameChar(startCh) ||
