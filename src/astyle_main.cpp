...
#define IS_PARAM_OPTION(arg, op) \
  ((arg).length() < strlen(op)   \
       ? 0                       \
       : (arg).COMPARE(0, strlen(op), string(op)) == 0)
#define IS_PARAM_OPTIONS(arg, a, b) \
  (IS_PARAM_OPTION((arg), (a)) || IS_PARAM_OPTION((arg), (b)))

#define GET_PARAM(arg, op) ((arg).substr(strlen(op)))
#define GET_PARAMS(arg, a, b) \
  (IS_PARAM_OPTION((arg), (a)) ? GET_PARAM((arg), (a)) : GET_PARAM((arg), (b)))
...
// Helper Functions
// --------------------------------------------------------------------------
void SetColor(unsigned short ForeColor = 3,
...
bool parseOption(ASFormatter &formatter, const string &arg,
  ...
  if ((arg == "n") || (arg == "suffix=none")) {
    ...
    if (suffixParam.length() > 0) _suffix = suffixParam;
...
void printHelpFull() {
  ...
  cout << "When indenting a specific file, the resulting indented file RETAINS "
          "the\n";
  cout << "original file-name. The original pre-indented file is renamed, with "
          "a\n";
  ...
  SetColor(7, 0);
  cout << "----------------\n";
  cout << "    Long options (starting with '--') must be written one at a "
          "time.\n";
  ...
  cout << "    -l OR --pad=block\n";
  cout << "    Enclose one statement in a begin-end only for keyword "
          "if/else/while/for.\n";
  cout << endl;
  cout << "    -P   OR  --pad=all\n";
  ...
  cout << "-------------\n";
  cout << "    --suffix=####\n";
  cout << "    Append the suffix #### instead of '.orig' to original "
          "filename.\n";
  cout << endl;
  cout << "    -n   OR  --suffix=none" << endl;
  cout << "    Tells Astyle not to keep backups of the original source files."
       << endl;
  cout << "    WARNING: Use this option with care, as Astyle comes with NO "
          "WARRANTY..."
       << endl;
  cout << endl;
  cout << "    -X   OR  --errors-to-standard-output\n";
  cout << "    Print errors and help information to standard-output rather "
          "than\n";
  ...
  cout << "    Parse used the specified options file: ####, options=none, "
          "none\n";
  ...
  if (shouldParseOptionsFile) {
    ...
    if (!optionsFileName.empty()) {
      ...
      if (optionsIn) {
        ...
        ok = parseOptions(formatter, fileOptionsVector.begin(),
  ...
  if (fileNameVector.empty()) {
    ...
    for (unsigned i = 0; i < fileNameVector.size(); i++) {
      ...
      while (formatter.hasMoreLines()) {
        ...
        if (formatter.hasMoreLines()) out << endl;
      }
